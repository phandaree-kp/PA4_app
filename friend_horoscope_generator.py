...
user_api_key = st.sidebar.text_input("🔑 กรอก OpenAI API Key ของคุณ", type="password")

prompt_template = """
สมมติว่าคุณเป็นนักพยากรณ์ดวงที่เน้นคำทำนายแบบขำขัน
...
for i in range(1, 6):
    ...
    behavior = st.text_input(f"นิสัยหรือพฤติกรรมเด่นของเพื่อนคนที่ {i}", 
                             placeholder="เช่น ขี้ลืม ชอบดูซีรี่ส์ ตอนเมาเคยอุ้มหมากลับบ้าน", key=f"behavior_{i}")
...
if st.button("🔮 ดูดวง!"):
    if not user_api_key:
        st.error("กรุณาใส่ API Key ก่อนนะงั้ฟ!")
    elif not friends_data:
        st.error("กรุณากรอกข้อมูลเพื่อนอย่างน้อย 1 คนน้า")
    else:
        st.info("⏳ กำลังสร้างคำทำนาย... โปรดรอสักครู่")
        ...
        for friend in friends_data:
            ...
            try:
                response = openai.ChatCompletion.create(
                    model="gpt-4o-mini", #รอดูอีกทีว่าจาซื้อ model ตุวไหน 
                    messages=messages
                )
